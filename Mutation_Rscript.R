...
msa_sel1 <- read.table("coocurnce_mutation_input.txt", header=TRUE, sep="\t") 
msa_sel1 <- read.table("Domain1_haplotypes_mutations_analysis_input.txt", header=TRUE, sep="\t") 
...
plot1 <- ggplot(melted_mat) + #add data and fill cells
  ...
  labs(title= paste("Co-ocurrence mutations in the Haplotypes of AMA1-Domain1" )) +
...
Protein <- 'Cooccurrence_mutations_AMA1_domain1_2nd.txt'
