...
seq<- read.dna("Domain1_445_906_Popgendata_BegCape_combined_subalign_subalign_subalign.fa", format="fasta") 
...
row.names(AMA1_Haplotypes) <- 1 : 93 #look at no accordingly change it
rownames(AMA1_Haplotypes) <- paste0('hap', rownames(AMA1_Haplotypes))

#view haplotypes differences
alview(AMA1_Haplotypes)

#Get and Write haplotype sequences
GetHaplo("Domain1_445_906_Popgendata_BegCape_combined_subalign_subalign_subalign.fa", align = NA, saveFile = TRUE, outname = "Haplotypes_seq_res.fa", format = "fasta",seqsNames = NA, silent = F)
...
all_haplotypes_diff <- as.data.frame(diffHaplo(AMA1_Haplotypes,1:93)) # no. can be changed after checking how many total haplotypes are there

#Write in a file
write.table(all_haplotypes_diff, file= "all_haplotypes_changes_first_domain.txt")
...
#Write into a file
write.table(ind.hap, file="individual_sample_haplotype_domain1.txt")
...
#Write into a file
write.table(new.hap, file="group_haplotype_domain1.txt")
...
plot(AMA1_Net, size = attr(AMA1_Net, "freq"), fast = F, show.mutation=2, labels=T, cex = 0.5, main = "Haplotype network for Begro and Capecoast", pie=new.hap, lwd = 0.5, scale=1)
...
network1 <- haploNet(haps1, getProb = TRUE)
...
plot(network1, size = attr(network1, "freq"), fast = F, show.mutation=2, labels=T, cex = 10, main = "", pie=new.hap, lwd = 0.5, scale=2)
legend(120,-50, colnames(new.hap), col=rainbow(ncol(new.hap)), cex=9, pch=20)
dev.off()
...
plot(network1, size = attr(network1, "freq"), fast = T, show.mutation=2, labels=F, cex = 0.5, main = "Haplotype network (freq >= 1) for Begro and Capecoast Domain1", pie=new.hap, lwd = 0.1, scale=1)

#Plot without labels of samples
plot(network1, size = attr(network1, "freq"), fast = T, show.mutation=2, labels=T, cex = 0.6, main = "Haplotype network (freq >= 1) for Begro and Capecoast Domain1", pie=new.hap, lwd = 0.5, scale=1)

#Plot proper network (fast=F) without labels
plot(network1, size = attr(network1, "freq"), fast = F, show.mutation=2, labels=F, cex = 0.5, main = "Haplotype network (freq >= 1) for Begro and Capecoast Domain1", pie=new.hap, lwd = 0.5, scale=1)

#Plot proper network (fast=F) with labels
plot(network1, size = attr(network1, "freq"), fast = F, show.mutation=2, labels=T, cex = 0.5, main = "Haplotype network (freq >= 1) for Begro and Capecoast Domain1", pie=new.hap, lwd = 0.5, scale=1)

#Plot proper network (fast=F) with labels
plot(network1, size = attr(network1, "freq"), fast = F, show.mutation=2, threshold=c(5,45), labels=T, cex = 0.5, main = "Haplotype network (freq >= 1) for Begro and Capecoast Domain1", pie=new.hap, lwd = 0.5, scale=1)
#Add legends
legend(20,40, colnames(new.hap), col=rainbow(ncol(new.hap)), pch=20)
...
#Add legends legend(180,40, colnames(new.hap), col=rainbow(ncol(new.hap)), pch=20)
...
network2 <- haploNet(haps2)


tiff("Domain1_freq2.tiff",
     width     = 10,
     height    = 10,
     units     = "in",
     res       = 150,
...
plot(network2, size = attr(network2, "freq"), fast = F, show.mutation=2, labels=T, cex = 10, main = "", pie=new.hap, lwd = 0.5, scale=2)
legend(120,-50, colnames(new.hap), col=rainbow(ncol(new.hap)), cex=9, pch=20)
dev.off()
...
#Plot without labels of samples, to quickly (fast=T), probably messy network
plot(network2, size = attr(network2, "freq"), fast = T, show.mutation=2, labels=F, cex = 0.5, main = "Domain1 Haplotype network (freq >= 2) for Begro and Capecoast", pie=new.hap, lwd = 0.1, scale=1)

#Plot without labels of samples
plot(network2, size = attr(network2, "freq"), fast = T, show.mutation=2, labels=T, cex = 0.5, main = "Domain1 Haplotype network (freq >= 2) for Begro and Capecoast", pie=new.hap, lwd = 0.5, scale=1)

#Plot proper network (fast=F) without labels
plot(network2, size = attr(network2, "freq"), fast = F, show.mutation=2, labels=F, cex = 0.5, main = "Domain1 Haplotype network (freq >= 2) for Begro and Capecoast ", pie=new.hap, lwd = 0.5, scale=1)

#Plot proper network (fast=F) with labels
plot(network2, size = attr(network2, "freq"), fast = F, show.mutation=2, labels=T, cex = 0.5, main = "Domain1 Haplotype network (freq >= 2) for Begro and Capecoast", pie=new.hap, lwd = 0.5, scale=1)
...
#Plot proper network (fast=F) with labels
plot(network3, size = attr(network3, "freq"), threshold = c(10,50), fast = F, show.mutation=2, labels=T, cex = 0.7, main = "Haplotype network (freq >= 3) for Begro and Capecoast", pie=new.hap, lwd = 0.5, scale=1)
